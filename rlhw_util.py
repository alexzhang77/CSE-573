# Gym was deprecated, so I needed to change the code
# to use the currently support version: https://gymnasium.farama.org
import time
import numpy as np
import torch
...
class Pytorch_Gym_Env(object):
	...
	def __init__(self, env_name, render_mode=None, device='cpu'):
	...
	def reset(self):
		return torch.from_numpy(self._env.reset()[0]).float().to(self._device).view(-1)
...
class Generator(object):
	...
	def __call__(self, horizon=None, render=False):
		...
		for _ in range(horizon):
			...
			state, reward, done, smt = self.env.step(actions[-1])
		...
		if self.drop_last_state:
			prev_state = torch.stack(states[:-1])
			states.pop(0)
		...
		return states, actions, rewards, prev_state
